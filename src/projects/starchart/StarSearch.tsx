...
export function StarSearch() {
  ...
  return (
    <div className="flex flex-col gap-1 relative">
      <span className="text-[11px] text-site-mid uppercase tracking-widest">Find</span>
      <input
        type="text"
        value={query}
        ...
        onKeyDown={onKeyDown}
        className="bg-transparent border border-site-line text-site-ink text-xs
          px-2 py-1.5 font-sans w-full
          placeholder:text-site-mid/60
      ...
      {showList && (
        <ul
          className="absolute top-full left-0 right-0 z-20 mt-0.5 max-h-64 overflow-y-auto
            custom-scrollbar bg-white border border-site-line shadow-lg"
        >
          {results.map((t, i) => (
            <li key={`${t.kind}:${t.label}:${t.ra.toFixed(3)}`}>
              <button
                ...
                className={`w-full text-left px-2 py-1.5 cursor-pointer flex items-baseline gap-1.5
                  ${i === active ? 'bg-site-ink/5' : ''}`}
              >
                <span className="text-[var(--project-accent)] text-[10px] shrink-0">
                  {KIND_MARK[t.kind]}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-[11px] text-site-ink uppercase tracking-wider truncate">
                    {t.label}
                  </span>
                  <span className="block text-[9px] text-site-mid truncate">{t.detail}</span>
                </span>
                <span className="text-[9px] text-site-mid font-mono shrink-0">
